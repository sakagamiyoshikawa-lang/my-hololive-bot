...
def main():
    ...
    <head>
        ...
        <style>
            ...
            header h1 {{ margin: 0; font-size: 3.5rem; font-weight: 900; text-shadow: 0 4px 10px rgba(0,0,0,0.3); }}
            
            .container {{ max-width: 1400px; margin: 0 auto; padding: 20px; }}

            .nav-box {{ background: white; padding: 25px; border-radius: 25px; margin: -40px auto 40px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.1); position: relative; z-index: 100; max-width: 900px; }}
            ...
            .btn-nav {{ padding: 12px 25px; border: 2px solid #eee; background: white; cursor: pointer; border-radius: 50px; font-weight: bold; transition: 0.3s; color: #666; }}
            ...
            .video-card.show {{ display: flex; flex-direction: column; animation: fadeIn 0.4s ease forwards; }}
            @keyframes fadeIn {{ from {{ opacity: 0; transform: translateY(10px); }} to {{ opacity: 1; transform: translateY(0); }} }}

            .grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 30px; }}
            .card {{ background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); transition: 0.3s; height: 100%; display: flex; flex-direction: column; }}
            ...
            .view-badge {{ position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.85); color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: bold; }}
            ...
            /* 楽曲情報ボックス */
            .music-card {{ background: #f3e5f5; border-radius: 12px; padding: 12px; margin-bottom: 15px; border-left: 6px solid var(--music); }}
            ...
            .btn-link {{ text-decoration: none; padding: 12px; border-radius: 12px; font-size: 0.8rem; font-weight: bold; text-align: center; transition: 0.2s; }}
    ...
    <body onload="update()">
        <header>
            <h1>🌟 {SITE_NAME}</h1>
            <p style="font-weight:bold; opacity:0.9;">AI楽曲解析 × 再生数TOP100ナビ</p>
        </header>
    ...
    for v in videos:
        # --- エラー防止ガード ---
        channel_data = v.get('channel')
        if not isinstance(channel_data, dict):
            continue
            
        ch_name = channel_data.get('name', 'Unknown')
        sub_org = channel_data.get('sub_org', '')
        
        # グループ判定ロジックの強化
        is_stars = "stars" in sub_org.lower() or "stars" in ch_name.lower() or "HOLOSTARS" in ch_name.upper()
        org_type = "holostars" if is_stars else "hololive"
        ...
        # AI解析：楽曲情報と紹介文
        prompt = f"Extract if it's music: [Song Name - Artist Name]. If not: [None]. Also, write 15-char summary. Format: Info | Summary. Title: {title}"
        try:
            res = client.models.generate_content(model='gemini-1.5-flash', contents=prompt)
            parts = res.text.strip().split('|')
            music_val = parts[0].strip()
            ai_desc = parts[1].strip() if len(parts) > 1 else "人気動画をチェック！"
        except:
            music_val, ai_desc = "None", "ホロライブ最新情報をAI分析"

        music_html = f'<div class="music-card"><span class="music-label">🎵 MUSIC INFO</span><div class="music-text">{music_val}</div></div>' if "None" not in music_val else ""

        # 収益リンク
        search = requests.utils.quote(f"{'ホロライブ' if org_type=='hololive' else 'ホロスターズ'} {ch_name}")
        amz_url = f"https://www.amazon.co.jp/s?k={search}&tag={AMAZON_ID}"
        rak_url = f"https://hb.afl.rakuten.co.jp/hgc/{RAKUTEN_ID}/?pc=https%3A%2F%2Fsearch.rakuten.co.jp%2Fsearch%2Fmall%2F{search}%2F"
    ...
    html_content += """
        ...
        <footer style="text-align: center; padding: 80px; color: #aaa; background: white; margin-top: 60px;">
            © 2026 ホロ活ナビ | AI Analysis Fan Project
        </footer>
    </body>
    </html>"""
