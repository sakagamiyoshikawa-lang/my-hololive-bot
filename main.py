...
def super_clean_name(raw_name):
    """名前からノイズを削ぎ落とし、日本語または英語のメイン呼称に一本化"""
    ...
    for ep in endpoints:
        ...
        try:
            ...
            if res.status_code == 200:
                data = res.json()
                # 組織IDが 'Hololive' であることを最終確認（Flayon等のStars-ENをここで弾く）
...
def main():
    ...
    def create_card(v):
        ...
        if status == 'upcoming' and raw_start:
            ...
            try:
                ...
                cal_url = f"https://www.google.com/calendar/render?action=TEMPLATE&text={urllib.parse.quote('【応援】'+clean_name)}&dates={st}/{et}&details=URL: https://www.youtube.com/watch?v={v_id}"
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="actions">
                    {main_btn}
                    <a href="https://twitter.com/intent/tweet?text={share_text}&url={SITE_URL}" target="_blank" class="btn share">📢 推しを布教する</a>
    ...
    <body>
        <header><h1>💙 {SITE_NAME}</h1><p>推しの活動をAIで見守り、全力で布教・支援する</p></header>
